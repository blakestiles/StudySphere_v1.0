      ...
      ...quizAttempts.map((q: Record<string, any>) => ({
        ...
        responses: (q.responses || []).map((r: any) => {
          ...
          return {
            ...
            questionText: r.questionText || question?.question || null,
            options: r.options?.length ? r.options : (question?.options || []),
            correctAnswer: r.correctAnswer ?? question?.correctAnswer ?? null,
            explanation: r.explanation || question?.explanation || null,
          };
        }),
      })),
